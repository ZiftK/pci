...
class DNode(ABC):
    ...
    def __init__(self, dir_name: str, *args, **kwargs) -> None:
        """
...
class NodeFactory:
    ...
    @staticmethod
    def get_node(node_type: DNodeTypes, name: str) -> DNode:
        """
        Return class reference of a node type specified by node_type
        """
        return NodeFactory._references.get(node_type, None)(name)
...
def queue_to_tree(lst: list) -> tuple[Tree, DNode]:
    ...
    while queue:
        ...
        d_child = NodeFactory.get_node(DNodeTypes(DNodeTypes(element.lower().split("_")[0])), element)
