...
def split_by(df: DataFrame, columns: list) -> list[DataFrame]:
    """
    Split the DataFrame into sub DataFrames with equal values for
    the corresponding columns and return a list with all the sub DataFrames
    """
...
class DataRange:
    ...
    def is_inside(self, value, column_name: str):
        ...
        """
        return self.min_val_from(column_name) <= value <= self.max_val_from(column_name)
